...
class passivation extends Controller
{
  public function processInsert()
  {
    ...
    DB::table('processes')->where('id',$processId)->update(['status'=>"pas_0"]);
    $processId=$processId;
  ...
  public function stageUpdate()
  {
   ...
   $processId=Input::get('process_id');
   DB::table('processes')->where('id',$processId)->update(['status'=>"pas_1"]);
 ...
 public function jigPopulate(){
  $results=\App\Process::where('status','que_1')->get();   
...
public function approvalPopulate()
{
  ...
  $results=\App\Process::where('status','pas_0')->get();
  
  foreach ($results as $data) {
    echo "<ons-list-item class='list__item ons-list-item-inner'><label>Jig number:".$data->jig_no."</label>
    <label>&nbsp;|&nbsp;</label>
    <label>Fixture: ".$data->fixture."</label>".'<input type="submit" value="Approve" status="pas" pid="'.$data->id.'" class="subbut approve">'."
